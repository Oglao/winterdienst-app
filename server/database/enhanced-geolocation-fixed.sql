-- Enhanced Geolocation with PostGIS Support (Fixed)
-- This file extends the existing schema with PostGIS functionality

-- Add missing columns to work_sessions if not exist
ALTER TABLE work_sessions ADD COLUMN IF NOT EXISTS distance_km DECIMAL(10,2);
ALTER TABLE work_sessions ADD COLUMN IF NOT EXISTS fuel_consumed DECIMAL(10,2);
...
-- Create or replace triggers to automatically update geometry
DROP TRIGGER IF EXISTS trigger_update_user_geometry ON users;
CREATE TRIGGER trigger_update_user_geometry
    BEFORE INSERT OR UPDATE ON users
    FOR EACH ROW EXECUTE FUNCTION update_user_geometry();

DROP TRIGGER IF EXISTS trigger_update_route_geometry ON routes;
CREATE TRIGGER trigger_update_route_geometry
    BEFORE INSERT OR UPDATE ON routes
    FOR EACH ROW EXECUTE FUNCTION update_route_geometry();

DROP TRIGGER IF EXISTS trigger_update_photo_geometry ON photos;
CREATE TRIGGER trigger_update_photo_geometry
    BEFORE INSERT OR UPDATE ON photos
    FOR EACH ROW EXECUTE FUNCTION update_photo_geometry();

DROP TRIGGER IF EXISTS trigger_update_weather_geometry ON weather_data;
...
SELECT 
    ...
    CASE 
        ...
        ELSE COALESCE(ws.distance_km * 1000, 0)
