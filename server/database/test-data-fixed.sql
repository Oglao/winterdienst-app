-- Test Data für PostGIS Integration (Fixed)
...
-- Testfahrzeuge (mit korrekten Spaltennamen)
INSERT INTO vehicles (license_plate, brand, model, year, fuel_type, tank_capacity, current_fuel_level, is_active) VALUES
...
-- Teststrecken mit Koordinaten in Hamburg (ohne Konflikt-Behandlung)
DELETE FROM routes WHERE name IN ('Hauptstraße Nord', 'Industriegebiet Süd', 'Wohngebiet West', 'Autobahn A7 Abschnitt');
...
-- Materialien für Salzverbrauch-Tests (ohne minimum_stock)
INSERT INTO materials (name, unit, current_stock, cost_per_unit) VALUES
('Streusalz', 'kg', 10000.0, 0.15),
('Splitt', 'kg', 5000.0, 0.25),
('Auftaumittel', 'liter', 2000.0, 1.20)
ON CONFLICT (name) DO UPDATE SET
  current_stock = EXCLUDED.current_stock;

-- Wetterdaten für Hamburg (mit korrekten Spalten)
INSERT INTO weather_data (location_name, latitude, longitude, temperature, humidity, wind_speed, weather_conditions, forecast_type, recorded_at) VALUES
('Hamburg Zentrum', 53.5511, 9.9937, -2.5, 85, 15.2, 'snow', 'current', CURRENT_TIMESTAMP),
('Hamburg Nord', 53.5711, 9.9937, -3.0, 88, 18.5, 'heavy_snow', 'current', CURRENT_TIMESTAMP),
('Hamburg Süd', 53.5311, 9.9937, -1.8, 82, 12.8, 'light_snow', 'current', CURRENT_TIMESTAMP);

-- Testfotos mit Standorten (ohne Konflikt-Behandlung)
...
-- Test Work Sessions (mit korrekten Spalten)
